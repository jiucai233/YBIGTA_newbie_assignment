...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    delimiter = b"\r\n\r\n"
    delimiter_index = raw.find(delimiter)
    if delimiter_index == -1:
        return None, "", "Invalid HTTP response: No header-body delimiter found"
    header = raw[:delimiter_index].decode("utf-8", errors="replace")
    body = raw[delimiter_index + len(delimiter):]
