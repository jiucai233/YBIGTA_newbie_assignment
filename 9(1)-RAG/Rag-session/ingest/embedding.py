...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    api_key = _get_api_keys()
    print(f'{api_key}')
    client = OpenAI(api_key=api_key[0], base_url=BASE_URL) 

    embedding_result = []
    total_batches = (len(texts) + BATCH_SIZE - 1) // BATCH_SIZE

    for i in tqdm(range(total_batches), desc="Embedding passages"):
        batch_texts = texts[i * BATCH_SIZE:(i + 1) * BATCH_SIZE]
        batch_embeddings = _embed_batch_safe(client, batch_texts)
        embedding_result.extend(batch_embeddings)

        if progress_callback:
            progress_callback(len(embedding_result), len(texts))
    os.makedirs(PROCESSED_DIR, exist_ok=True)
    np.save(EMBEDDINGS_PATH, np.array(embedding_result, dtype=np.float32))
    IDS_PATH.write_text(json.dumps(ids[:len(embedding_result)]))  
...
def embed_query(query: str) -> list[float]:
    ...
    api_key = _get_api_keys()
    clients = [OpenAI(api_key=key, base_url=BASE_URL) for key in api_key]
    truncated_query = _truncate(query)
    response = clients[0].embeddings.create(
